...
const moviesRoutes = require('../routes/v1/moviesRoutes');
...
it("TEST 2 : ------------------> Should return movies for a valid year", async () => {
    const res = await request(app).get('/api/v1/movies?year=2020');
    expect(res.statusCode).toBe(200);
...
it("TEST 9 : ------------------> Should return movie details for a valid imdbId", async () => {
    const res = await request(app).get('/api/v1/movies/tt0111161'); // The Shawshank Redemption
    expect(res.statusCode).toBe(200);
